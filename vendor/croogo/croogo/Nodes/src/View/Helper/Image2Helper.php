...
class Image2Helper extends Helper {
    ...
    public function source($path = '', $absolute = false) {
        ...
        // debug(ROOT . DS . 'webroot');
        if (!$absolute) {
            $path = ROOT . DS . 'webroot' . $path;
        }
     ...
     * @return object
     */
    public function crop($width, $height, $resize = true, $position = '') {
        ...
        if( $position == 'top2' ) { $start_x = 0; $start_y = 0; } else { $position = ''; }

        $this->_nativeResize($start_x, $start_y, $width, $height, 'crop'.$position);
        return $this;
    }
    ...
    protected function _nativeResize($start_x, $start_y, $width, $height, $method = 'na') {
        ...
        $cache_dir = implode(DS, ['uploads', 'resized']);
        $cache_dir = ROOT . DS . 'webroot' . DS . $cache_dir . DS;
    ...
    public function watermark($watermark_path, $position = 'center', $watermark_absolute_path = false) {
        ...
        $cache_dir = implode(DS, ['uploads', 'resized']);
        $cache_dir = ROOT . DS . APP_DIR . DS . basename(dirname(__FILE__))
            . DS . $cache_dir . DS;
    ...
    public function imagePath() {
        ...
        $cache_dir = implode('/', ['uploads', 'resized']);
    ...
    public function resize($path, $width, $height, $method = 'resizeRatio', $htmlAttributes = array(), $return = false, $server_path = false) {
        ...
        $fullpath = ROOT . DS . APP_DIR . DS . basename(dirname(__FILE__))
            . DS . $uploadsDir . DS;
        if (!$server_path) {
            $url = ROOT . DS . APP_DIR . DS . basename(dirname(__FILE__))
                . DS . $path;
        } else {
            $url = $server_path;
        }
